...
static struct battery_parameter g24_battery = {
    ...
    .pmu_battery_rdc =  79,////73,     ////150,
    .pmu_battery_cap = 3660,    ////8000,
...
{	
	gpio_out(PAD_GPIOE_10, 1); //set back camera power disable
...
{
    gpio_out(PAD_GPIOE_11, 1);    // set front camera power disable
...
#ifdef CONFIG_FOCALTECH_CAPACITIVE_TOUCHSCREEN_G24M1306CM
...
#endif
#ifdef CONFIG_GSl1680_CAPACITIVE_TOUCHSCREEN_G24M1306CM
    {
...
static struct i2c_board_info __initdata aml_i2c_bus_info_b[] = {
...
#ifdef CONFIG_SND_SOC_WM8960
    {
        ...
        .platform_data = &wm8960_codec_data,
    },
#endif
...
//set status
	...
	gpio_set_status(PAD_GPIOAO_3,gpio_status_out);
...
{
	DBG_LINE_INFO();
    gpio_set_status(PAD_GPIOAO_3,gpio_status_out);//set wifi_en gpio mode out
    if(is_on){
		gpio_out(PAD_GPIOAO_3,1);
		printk("WIFI  Enable! \n");
    		}
	else{
		gpio_out(PAD_GPIOAO_3,0);
...
static void bt_device_init(void)
{
    ...
    /* BT_WAKE */
    gpio_set_status(PAD_GPIOA_1,gpio_status_out);
...
#if defined(CONFIG_SENSORS_MMA7660) || defined(CONFIG_SENSORS_MC32X0)
...
MACHINE_START(MESON6_G24_M1306CM, "Amlogic Meson6 g24 M1306CM customer platform")
