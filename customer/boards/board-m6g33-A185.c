...
static struct battery_parameter g24_battery = {
    ...
    .pmu_battery_rdc = 87,     ////150,
    .pmu_battery_cap = 7200,    ////8000,
...
static int gc0308_init(void)
{
    ...
    gpio_out(PAD_GPIOE_10, 1);    // set camera power enable
    msleep(20);
    ...
    // set camera power enable
    gpio_out(PAD_GPIOE_10, 0);    // set camera power enable
    msleep(20);
...
#elif  defined CONFIG_SPI_NAND_COMPATIBLE
	{
        .name = NAND_BOOT_NAME,
        .chip_enable_pad = AML_NAND_CE0,
        .ready_busy_pad = AML_NAND_CE0,
...
static pinmux_item_t rt5616_pinmux[] = {
...
static pinmux_set_t rt5616_pinmux_set = {
    .chip_select = NULL,
    .pinmux = &rt5616_pinmux[0],
};

static void rt5616_device_init(void)
{
    /* audio pinmux */
    pinmux_set(&rt5616_pinmux_set);

    /* GPIOA_19 PULL_UP_REG0 bit19 */
    aml_set_reg32_bits(P_PAD_PULL_UP_REG0, 1, 19, 1);
}

static void rt5616_device_deinit(void)
{
    pinmux_clr(&rt5616_pinmux_set);
}

static int rt5616_hp_detect(void)
{
#define HP_PLUG     1
#define HP_UNPLUG   0
    ...
    /* GPIOA_19 */
    aml_clr_reg32_mask(P_PAD_PULL_UP_REG0,1<<19);	
...
if(val == HP_PLUG)
{
//	printk("wanghai: %s:HP_PLUG\n", __func__);
	gpio_out(PAD_GPIOC_4, 0); 
    val = HP_PLUG;
}
else
{
//	printk("wanghai: %s:HP_UNPLUG\n", __func__);
...
static struct rt5616_platform_data rt5616_pdata = {
    .hp_detect      = rt5616_hp_detect,
    .device_init    = rt5616_device_init,
    .device_uninit  = rt5616_device_deinit,

    .spk_watt       = RT5616_SPK_1_0W,
    .spk_output     = RT5616_SPK_STEREO,
    .mic_input      = RT5616_MIC_DIFFERENTIAL,
};

static struct platform_device aml_rt5616_audio = {
    .name           = "aml_rt5616_audio",
    .id             = 0,
    .resource       = aml_m6_audio_resource,
    .num_resources  = ARRAY_SIZE(aml_m6_audio_resource),
    .dev = {
        .platform_data = &rt5616_pdata,
    },
};
