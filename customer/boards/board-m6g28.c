...
#include <mach/pm.h>
#include <mach/voltage.h> //fix@letv
#include <linux/uart-aml.h>
#include <linux/i2c-aml.h>

#include "board-m6g28.h"
...
#define STREAMBUF_MEM_SIZE          (SZ_1M*18)
...
//fix@letv
#ifdef CONFIG_AMLOGIC_SETIO     
#include <linux/aml_setio.h>
static unsigned io_status =1;
void extern_net_led(int is_power)
{
	aml_clr_reg32_mask(P_AO_RTI_PIN_MUX_REG, ((1<<5)|(1<<1)|(1<<23))); //disable pinmux
	aml_clr_reg32_mask(P_AO_GPIO_O_EN_N,1<<5);     //GPIO_AO 5 output

	//fix@letv 20130324 by alexwang
	uint32_t board_type = 0;
	
    board_type = aml_get_reg32_bits(AOBUS_REG_ADDR(0x28), 8, 3);
	printk("extern_net_led board_type = %d\n", board_type);
    if(board_type == 0x3){ /*c1s*/
		if(is_power != 0){
        	is_power = 0;
		}
		else{
	...
	printk("extern_net_led is_power = %d\n", is_power);
	//endfix		
	if(is_power)
		aml_set_reg32_mask(P_AO_GPIO_O_EN_N,1<<21);     //GPIOAO_5 H NET_LED  Green
	else
		aml_clr_reg32_mask(P_AO_GPIO_O_EN_N,1<<21);     //GPIOAO_5 L NET_LED  Red
	
	printk(KERN_INFO "**********************set net led %d\n",is_power);
}
EXPORT_SYMBOL(extern_net_led);

static unsigned aml_8726m_setio_status(unsigned status)
{
	return io_status;
}

static void aml_8726m_setio_val(int val)
{
	extern_net_led(val);
}

static void aml_8726m_setio_init(void)
{
	aml_8726m_setio_val(1);
}

struct aml_setio_platform_data aml_setio_platform =
{
    .setio_init = aml_8726m_setio_init,
    .setio_val = aml_8726m_setio_val,
    .io_status = aml_8726m_setio_status,
};

static struct platform_device aml_setio_device = {
    .name = "aml-setio",
    .id = -1,
    .num_resources = 0,
    .resource = NULL,
    .dev = {
        .platform_data = &aml_setio_platform,
    },
};
#endif
//end
...
static struct aml_uart_platform  __initdata aml_uart_plat = {
    ...
    .pinmux_uart[1] = (void*)&aml_uart_a, //fix@letv alexwang 20130319
    .pinmux_uart[2] = NULL,
    .pinmux_uart[3] = NULL,
    .pinmux_uart[4] = NULL
};
...
{
	uint32_t board_type = aml_get_reg32_bits(AOBUS_REG_ADDR(0x28), 8, 3);
    if(board_type != 0x7){
		/* BT_RST_N */
...
static struct aml_card_info meson_card_info[] = {
    ...
    [1] = {
        ...
        .card_power_en_reg  = 0, //fix@letv alexwang 20130319
        .card_power_en_mask = 0,
        .card_power_output_reg  = 0,
        .card_power_output_mask = 0,
        .card_power_en_lev  = 0,
        .card_wp_en_reg     = 0, //endfix
        .card_wp_en_mask    = 0,
        .card_wp_input_reg  = 0,
...
//#ifdef CONFIG_SMP    
#ifndef CONFIG_SUPPORT_USB_BURNING
...
{   
	//fix@letv
	if(is_power) {
        printk("usb_wifi_power: on.\n");
        aml_set_reg32_bits(CBUS_REG_ADDR(0x2008),0,11,1);   //GPIOE_11 mode
        aml_set_reg32_bits(CBUS_REG_ADDR(0x2009),0,11,1);   //GPIOE_11 level
	}
	else {
	 	printk("usb_wifi_power: off.\n");
        aml_set_reg32_bits(CBUS_REG_ADDR(0x2008),0,11,1);   //GPIOE_11 mode
        aml_set_reg32_bits(CBUS_REG_ADDR(0x2009),1,11,1);   //GPIOE_11 level
	}
	//end
	return 0;
}
...
static void m6ref_set_vccx2(int power_on)
{
    if (power_on) {
        ...
        aml_set_reg32_bits(SECBUS2_REG_ADDR(0), 1, 0, 1);
        aml_set_reg32_bits(AOBUS_REG_ADDR(0x24), 0, 31, 1);
				
				//fix@letv
				aml_clr_reg32_mask(P_AO_GPIO_O_EN_N,1<<4);//GPIO_AO 4 output
				aml_set_reg32_mask(P_AO_GPIO_O_EN_N,1<<20);//GPIO_AO 4  off standby-led    
				//end    
    }
    else {
        ...
        //fix@letv
				aml_clr_reg32_mask(P_AO_GPIO_O_EN_N,1<<4);//GPIO_AO 4 output
				aml_clr_reg32_mask(P_AO_GPIO_O_EN_N,1<<20);//GPIO_AO 4 on standby-led        
				//end
    }
    //fix@letv
    #ifdef CONFIG_AM_WIFI_USB
...
static unsigned int vcck_pwm_table[MESON_CS_MAX_STEPS] = {
	...
	0x060016, 0x090013, 0x0b0011, 0x0b0011, 
	0x0f000d, 0x11000b, 0x130009, 0x11000b, 
	0x170005, 0x190003, 0x1b0001, 0x170005,  
};
static unsigned int vcck_pwm_table_c1s[MESON_CS_MAX_STEPS] = {
	0x00001c, 0x01001b, 0x030019, 0x040018, 
...
//    printk("***vcck: set_voltage");
	uint32_t board_type = aml_get_reg32_bits(AOBUS_REG_ADDR(0x28), 8, 3);
	if(board_type == 0x3){
    	aml_write_reg32(P_PWM_PWM_C, vcck_pwm_table_c1s[level]);
	} else {
...
static void vcck_pwm_init() {
	...
	if(board_type == 0x3){
		aml_write_reg32(P_PWM_PWM_C, vcck_pwm_table_c1s[0]);
	} else {
...
static struct meson_cs_pdata_t vcck_pdata = {
    ...
    .voltage_step_table = {
        1380000, 1370000, 1350000, 1330000,
        1300000, 1240000, 1230000, 1210000,
        1180000, 1160000, 1130000, 1110000,
        1080000, 1060000, 1030000, 1010000,
    },
    .default_uV = 1300000,
...
static struct meson_opp vcck_opp_table_c1s[] = {
    /* freq must be in descending order */
    {
        .freq   = 1500000,
        .min_uV = 1300000,
...
static unsigned int vcck_cur_max_freq()
{
	...
	uint32_t board_type = aml_get_reg32_bits(AOBUS_REG_ADDR(0x28), 8, 3);
	if(board_type == 0x3){
    	return meson_vcck_cur_max_freq(vcck, vcck_opp_table_c1s, ARRAY_SIZE(vcck_opp_table_c1s));
	} else {
...
static int vcck_scale(unsigned int frequency)
{
	...
	uint32_t board_type = aml_get_reg32_bits(AOBUS_REG_ADDR(0x28), 8, 3);
	if(board_type == 0x3){
    	return meson_vcck_scale(vcck, vcck_opp_table_c1s, ARRAY_SIZE(vcck_opp_table_c1s), frequency);
	} else {
...
static int _key_code_list[] = {KEY_KPEQUAL,KEY_POWER}; //modified by lixin:2012-12-14
...
#endif
//fix@letv
#if defined(CONFIG_AMLOGIC_SETIO)    
        &aml_setio_device,
 #endif
 //end
#ifdef CONFIG_CPU_FREQ
...
//    meson_cache_init();
    ...
    aml_set_reg32_bits(AOBUS_REG_ADDR(0x24), 0, 21, 1);
    aml_set_reg32_bits(AOBUS_REG_ADDR(0x24), 0, 31, 1);
	//fix@letv alexwang 20130319
#ifdef CONFIG_AM_WIFI
    wifi_dev_init();
...
#define FEC_B
   ...
   m6skt_smc_init();
    ...
    //net red led
    aml_clr_reg32_mask(P_AO_RTI_PIN_MUX_REG, ((1<<5)|(1<<1)|(1<<23)));
    aml_clr_reg32_mask(P_AO_GPIO_O_EN_N,1<<5);//GPIO_AO 5 output
	//fix@letv alexwang 20130319
    board_type = aml_get_reg32_bits(AOBUS_REG_ADDR(0x28), 8, 3);
    if(board_type == 0x3){
        aml_set_reg32_mask(P_AO_GPIO_O_EN_N,1<<21);//GPIO_AO 5 H NET_LED RED
    }else{
        aml_clr_reg32_mask(P_AO_GPIO_O_EN_N,1<<21);//GPIO_AO 5 H NET_LED RED
    }
	//endfix
		//fix@letv
		aml_clr_reg32_mask(P_AO_RTI_PIN_MUX_REG, ((1<<6)|(1<<2)|(1<<24)));
		aml_clr_reg32_mask(P_AO_GPIO_O_EN_N,1<<4);//GPIO_AO 4 output
		aml_set_reg32_mask(P_AO_GPIO_O_EN_N,1<<20);//GPIO_AO 4  off standby-led    
		//end
