...
static int __init early_mem(char *p)
{
...
#if defined(CONFIG_PLAT_MESON)
	{
		...
		if (machine_desc &&
			...
			/* 0M - 64M */
			printk("%s:%d: start=0x%x, size=0x%x\n", __FUNCTION__, __LINE__, bankstart, vstart-bankstart);
		...
		if (bankstart < start + size) {
			...
			firmwaresize = SZ_1M + SZ_1M + SZ_64M;			
			firmwaresize += meson_trustzone_getmemsecure_size();
#else
			firmwaresize = SZ_1M;
#endif
			...
			/* 170M - 511M */
			printk("%s:%d: start=0x%x, size=0x%x\n", __FUNCTION__, __LINE__, bankstart, firmwarestart-bankstart);
		...
		if (bankstart < start + size){
			printk("%s:%d: start=0x%x, size=0x%x\n", __FUNCTION__, __LINE__, bankstart, start+size-bankstart);
