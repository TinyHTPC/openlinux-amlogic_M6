	...
	if (enable >= 1) {
		...
		case 1:
			...
			val = 0 << 0  //Power Down
				...
				| 0;
			IO_WRITE32(val, (np->base_addr + ETH_MAC_PMT_Control_and_Status));
			break;
		case 2:
			val = 0 << 0  //Power Down
			...
			/* Filter 0 */
			val = 0x7f;
			IO_WRITE32(val, (np->base_addr + ETH_MAC_Remote_Wake_Up_Frame_Filter));
			val = 0;
			/* Filter 1,2,3 */
			for (i = 0; i < 3; i++) {
				IO_WRITE32(val, (np->base_addr + ETH_MAC_Remote_Wake_Up_Frame_Filter));
			...
			for (i = 0; i < 8; i++) {
				val = IO_READ32(np->base_addr + ETH_MAC_Remote_Wake_Up_Frame_Filter);
...
static int phy_reset(struct net_device *ndev)
{
	...
	if(np->phy_Identifier == PHY_IC_IP101ALF){
		...
		mdio_write(ndev, np->phys[0], 0x1a, 0x1252);
	...
	printk("Current DMA mode=%x, set mode=%lx\n", IO_READ32(np->base_addr + ETH_DMA_6_Operation_Mode), val);
...
static int probe_init(struct net_device *ndev)
{
	...
	if(priv->mii == PHY_IC_IP101ALF){
		...
		mdio_write(ndev, priv->phys[0], 0x1a, 0x1252);
	}
